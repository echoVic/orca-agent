...
use crate::lifecycle::{
    ...
    RuntimeToolActorContext, RuntimeWorkflowIpc,
};
...
impl RuntimeToolActorContext {
    pub fn classify_dispatch(
        ...
        match request.name {
            ...
            ToolName::SubagentMessage => RuntimeSpecialToolDispatch::SubagentMessage,
    ...
    /// Queues guidance for a running child agent.
    ///
    /// This takes no execution slot: steering work in flight must stay
    /// possible while the scope is saturated.
    pub fn execute_subagent_message_tool(
        &mut self,
        request: &ToolRequest,
        task_registry: &TaskRegistry,
    ) -> ToolResult {
        ...
        let Some(message) = args
            ...
            return ToolResult::invalid_input(request, "missing required field: message");
        };
        if task_registry.get(task_id).is_none() {
            return ToolResult::failed(request, format!("task '{task_id}' not found"), None);
        }
        match task_registry.queue_child_message(task_id, message) {
            Ok(queued) => ToolResult::completed(
                request,
                json!({
                    "task_id": task_id,
                    "message_id": queued.message_id,
                    "status": "queued_for_child",
                    "pending_messages": task_registry.pending_child_message_count(task_id),
                    "note": "the child reads this at its next safe boundary; it keeps its original brief",
                })
                .to_string(),
                false,
            ),
...
fn task_summary_json(task: BackgroundTaskSummary) -> Value {
    json!({
        ...
        // Ownership: `workspace` means the user asked to keep it running, so
        // it is not a leftover to clean up at the end of the task.
        "lifetime": task.lifetime.as_str(),
