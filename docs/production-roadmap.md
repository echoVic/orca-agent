...
Active/approval/failed and rich-task graph reconstruction is the next cold
migration boundary. Public task, CLI, server/JSONL, ACP, and persisted schemas
are unchanged.
...
1. **ThreadActor is still a god object.** At the audited `HEAD`,
   `crates/orca-runtime/src/runtime_host.rs` is 51,787 lines. Its
...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   ...
   error precedence. The first cold legacy registry slice now imports only
   recorded, immutable MainSession Completed/Stopped/Cancelled history under a
   locked receipt and dedicated coordinator authority. Active, approval,
   failed/retryable, workflow, subagent, shell, and monitor rows still require
   durable operation, interaction, ownership, graph, and rollback semantics
   before they can become visible or actionable. Live
