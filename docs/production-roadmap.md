...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established thirty-seven focused owners/boundaries so far (insert-escape, presentation,
   ...
   terminal session startup ownership, and renderer input-wake ownership);
  `app.rs` is currently 7,880 lines, `renderer_input_wake.rs` 298 lines,
