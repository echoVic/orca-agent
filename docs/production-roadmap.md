...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established thirty-five focused owners/boundaries so far (insert-escape, presentation,
   ...
   ownership, renderer runtime-event ownership, and renderer frame ownership);
  `app.rs` is currently 7,952 lines, `renderer_frame.rs` 525 lines,
