...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established forty focused owners/boundaries so far (insert-escape, presentation,
   ...
   renderer input-routing ownership, and renderer interaction-acknowledgement
   ownership, and renderer runtime-inbox ownership); `app.rs` is currently
   7,778 lines, `renderer_runtime_inbox.rs` 104 lines,
