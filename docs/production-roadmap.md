...
and iteration short-circuiting; the outer loop keeps composer synchronization,
exit handling, presentation, and shutdown. No event/action, runtime surface,
...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established forty-one focused owners/boundaries so far (insert-escape, presentation,
   ...
   ownership, renderer runtime-inbox ownership, and renderer iteration-event
   routing ownership); `app.rs` is currently 7,759 lines,
   ...
   `runtime_event_actions.rs` 1,248 lines, and `types.rs` 8,936 lines. Outer
   renderer-loop orchestration plus cold legacy registry reconciliation remain open; live
