...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established thirty-six focused owners/boundaries so far (insert-escape, presentation,
   ...
   ownership, renderer runtime-event ownership, renderer frame ownership, and
   terminal session startup ownership); `app.rs` is currently 7,917 lines,
   `terminal_session.rs` 207 lines, `renderer_frame.rs` 525 lines,
