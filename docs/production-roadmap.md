...
| Shell execution | `bash` is the only model-facing command launcher and every command is a runtime-owned task. `yield_time_ms` bounds one call without killing the command; `timeout_ms` is the caller execution deadline. `task_read_output`, `task_send_input`, `task_wait`, and `task_stop` use one task id across pipe and PTY commands. The bounded-mailbox `TerminalService` supervisor reaps natural exits and stops without polling and retains output for cursor reads | Codex `exec_command` streaming/exit watcher; Grok Build exit watcher and completion notification | Implemented |
...
| Tool output control | Bounded UTF-8 memory tails plus session-scoped disk output archives, explicit caller-owned cursors through `task_read_output`, omission metadata, process-restart reads, and bounded per-task/session/chunk retention | Codex bounded exec replay plus disk-backed task output and offset polling | Implemented; original shell/process state is not resumed |
| Custom agents | User/trusted-project Markdown and parsed YAML definitions, model-visible catalog, inherited tool intersection, immutable definition/delegation snapshots, and one queued task protocol across direct, nested, Workflow, hosted, continued, and recovered execution. Built-in read-only roles have runtime-enforced tool and evidence ceilings | Claude/Grok declarative agents | Implemented; parent-restart recovery and real-model parallel investigation verified |
...
| Model metadata | `ModelSelection`, canonical `deepseek-flash` plus retired-name normalization, typed direct-vs-analysis image routing, direct Flash image input, and Flash visual preprocessing for `auto`/Pro | Codex `models-manager` with model capability metadata | Partial |
...
| Workflows | JavaScript workflow DSL, generated drafts, edit/save/run controls, reusable workflow commands, task state, notifications, runtime status events, evidence-bound reports, and worktree-isolated/recoverable agent runs. Workflow children share the root task tree's execution scope, queue, deadline, and persistent budget settlement | Codex/Claude workflow orchestration concepts | Implemented |
| Runtime lifecycle | Headless, server-mode, and TUI runs use one runtime-owned task lifecycle. `ExecutionScope` serializes durable admission across processes, limits one root task tree to 32 active child leases by default, queues accepted overflow, lets safely waiting parents yield capacity, and recovers unstarted launch intents without replaying indeterminate side effects. `RuntimeThread` and focused controllers retain operation, interaction, Goal, task, and workflow ownership; external processes and search workers keep explicit cleanup ownership | Codex `Session -> Task -> Turn`, app-server request processors; Grok queued agent admission | Implemented |
