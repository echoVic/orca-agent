...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   stream has established thirty-eight focused owners/boundaries so far (insert-escape, presentation,
   ...
   terminal session startup ownership, renderer input-wake ownership, and
   renderer input-routing ownership); `app.rs` is currently 7,782 lines,
