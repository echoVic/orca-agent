...
3. **TUI/runtime protocol drift is being sliced.** The `codex/tui-convergence`
   ...
   error precedence. Cold legacy registry reconciliation remains a separate
   runtime/persistence Spec requiring durable typed identity, operation,
   interaction, ownership, and rollback semantics before registry-only rows can
   become visible or actionable. Live
