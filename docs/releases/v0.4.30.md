...
- Gates background tool-stream fixtures on a committed surface handoff instead
  of a scheduler-dependent fixed delay.
- Gives real workflow process cleanup a dedicated cross-platform completion
  bound while retaining the subprocess and event-sequence assertions.
...
- Batches high-throughput shell capture into 64 KiB durable chunks, preserving
  full-sync SQLite writes while avoiding thousands of tiny transactions.
